...
func main() {
	...
	log.Info().Str("address", listenAddr).Msg("listening for game service connections")
	...
	if err := gameServer.Listen(ctx, listenAddr); err != nil {
		log.Fatal().Err(err).Msg("error in game server")
	}
}
